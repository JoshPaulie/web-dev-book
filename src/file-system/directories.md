...
## Directory hierarchy
Because folders are nested inside one another, a hierarchy of folders begins to form. This hierarchy consists of two roles: the child and parent.

Let's visualize how our homework assignments are structured to better understand the file system hierarchy.

```
$ tree ~/web-dev/assignments/hw0
.
├── index.html
├── pages/
│   ├── about.html
...
Now note how `about.html` and `blog.html` "branch off" of the `pages/` directory. This would mean that the html pages are children to the `pages/` directory, which is itself a child to the root directory.
